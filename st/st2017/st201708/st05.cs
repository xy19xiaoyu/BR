...
where cn.pdy <='{1}' and cn.type='发明专利' and cn_lg.dead_year >{1} and cn_zt.ztname in ({0}) {2}
...
PIVOT (sum(申请量) for 申请人类型 in([工矿企业],[科研院所],[个人])) as table2";
                ...
                {
                    string exe_sql = "";
                    exe_sql = string.Format(sql, hy.Value, year, GetFilter());
