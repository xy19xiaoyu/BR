...
{
    public class st26 : ST_Base
    {

        public st26()
        {
            this.Id = 26;
            this.Name = "(26)城市-申请人类型-有效发明专利量";
            this.type = "CN";
            this.DES = "";
        }
...
where cn.shi='{0}' and cn.type= '发明专利' and cn.pdy <='{1}'  and cn_lg.dead_year >{1}
group by cn_pa.pa_type) as a
PIVOT (sum(申请量) for 申请人类型 in([大专院校],[个人],[工矿企业],[科研单位],[事业单位])) as table2";
                List<int> values = new List<int>();
                foreach (var year in config.Years)
                {
                    string exe_sql = string.Format(sql, name, year);
                ...
                val_row.CreateCell(0).SetCellValue(name.ToString());
